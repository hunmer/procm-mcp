...
export async function listRooms(): Promise<RoomView[]> {
  const response = await api<{ rooms: RoomView[] }>("GET", "/api/rooms");
...
// The server's own debug.log state: effective size cap and every on-disk
// debug.log file in the data root (any server generation).
export function getServerLogInfo(): Promise<ServerLogInfo> {
  return api<ServerLogInfo>("GET", "/api/server-log");
}

// Set (positive integer bytes) or clear (null) the persisted debug.log size
// cap. Clearing falls back to env PROCM_DEBUG_LOG_MAX_BYTES, else 20MB.
// Returns the refreshed state.
export function updateServerLogMaxBytes(
  maxBytes: number | null,
): Promise<ServerLogInfo> {
  return api<ServerLogInfo>("PUT", "/api/server-log/settings", { maxBytes });
}

// Remove every server-log directory in the data root (the current
// generation's dir is emptied in place). Returns the handled folder names.
export function clearServerLogs(): Promise<{ cleared: string[] }> {
  return api<{ cleared: string[] }>("DELETE", "/api/server-log");
}
