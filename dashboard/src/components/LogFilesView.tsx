...
import { loadShowJson } from "./log-panel/constants";
...
export function LogFilesView({
  ...
  const [showJson] = useState(loadShowJson);
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 overflow-hidden", className)}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <ScrollArea className="log-selectable bg-zinc-950 min-h-0 flex-1 text-zinc-300">
          <div className="min-h-full p-4">
            ...
            {contentError ? (
              <p className="m-0 whitespace-pre-wrap break-words font-mono text-xs leading-relaxed">
                <span className="text-red-400">
                  {t("logFiles.contentError", { message: contentError })}
                </span>
              </p>
            ) : contentLoading ? (
              <div className="flex min-h-[200px] items-center justify-center">
                <Spinner className="text-zinc-500 size-5" />
              </div>
            ) : entries.length === 0 ? (
              <Empty className="min-h-[200px] text-zinc-400">
                <EmptyHeader>
                  ...
                  <EmptyTitle className="text-zinc-300">
                    {t("logFiles.contentEmpty")}
                  </EmptyTitle>
                  <EmptyDescription className="text-zinc-500">
                    {t("logFiles.contentEmptyDesc")}
                  </EmptyDescription>
                </EmptyHeader>
              </Empty>
            ) : (
