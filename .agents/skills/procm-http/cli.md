...
Room messages and process logs are separate channels:

- Use room `publish`/`subscribe` for unit/integration test coordination, RPC,
  fixture results, and explicit event notifications.
- Use `logs`, `grep`, and the dashboard `LogPanel` for application
  `console.*` output. These read the owning process's stdout/stderr.
- Browser or miniapp output belonging to a hosted Web process must be sent to
  that process's local bridge and written to its terminal output.
- `roomId` identifies a process's room context; it does not redirect terminal
  output. Never publish every console call to `$procm/log`.
