...
## 2026-08-25 — 增量更新（rest.ts 增 clearLogs）

- **背景**：8-20 `rest.ts` 新增 `clearLogs(client, id?)` 便捷封装（`id` 缺省取 `client.processId` 操作自身进程），随 0.1.1（8-24 发布）上线；源码其后无改动。
- **更新**：`CLAUDE.md`（rest.ts 描述 + 扫描状态）、`module-responsibilities`/`overview`/`file-map`/`public-interfaces` 各补 `clearLogs` 一处。
