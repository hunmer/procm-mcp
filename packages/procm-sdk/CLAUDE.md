...
- **更新时间**：2026-08-25
- **已扫描**：`src/` 全部 8 文件（8-15/8-17 两轮 100% 逐行；8-20 的 `rest.ts` 增量 `clearLogs()` 本轮经 diff 核实，已随 0.1.1 发布）、`package.json`、`tsconfig.json`；`dist/` 抽查（与 src 同构）；消费方（根 `src/room-hub.ts`、`tests/room-sdk.mjs` 等）在根索引覆盖。
- **跳过**：`dist/*.map`（产物）。
