...
- 必须确认进程管理工具连接的就是用户的 `7331` 实例。若机器上存在多个 `procm-mcp`，工具启动的进程可能写入另一实例的数据目录；这类进程不会出现在 `7331/api/processes`。排查时同时核对端口、PID、`/api/processes` 返回的 `serverId` 和进程管理器返回的进程列表。
