...
对：stdio 14 个，`/mcp` 13 个（仅缺 `process-input`；api-operations 组三件 8-18 起已在 `/mcp` 注册）。`process-input` 仅在 `index.ts` 注册，未在 `mcp-http.ts` 的 `registerAllTools` 注册。要写 stdin/发信号经 HTTP 请用 REST `POST /api/processes/:id/input` 或 dashboard。

**Q：`debug.log` 越来越大怎么办？**
单文件有大小上限：数据根 `settings.json`（经 `PUT /api/server-log/settings` 或 dashboard 设置页写入）> `PROCM_DEBUG_LOG_MAX_BYTES` > 默认 20MB，超限整体截断重写。历史各实例日志用 `DELETE /api/server-log` 清理（当前实例目录原地清空）。注意：**进程的 `.log` 没有上限**。
