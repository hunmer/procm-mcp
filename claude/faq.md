...
对：stdio 5 个，`/mcp` 4 个（缺 `process-input`）。`process-input` 仅在 `index.ts` 注册，未在 `mcp-http.ts` 的 `registerAllTools` 注册。要写 stdin/发信号经 HTTP 请用 REST `POST /api/processes/:id/input` 或 dashboard。
