...
| `PROCM_DEBUG_LOG_MAX_BYTES` | `debug.log` 单文件大小上限（正整数字节） | 20971520（20MB）；优先级低于数据根 `settings.json` 的持久化设置（经 `PUT /api/server-log/settings` 写入） |
...
| `procm-commands.json` | 本仓自用命名命令定义（全局 `procm-mcp` 按名启动，见 debug.md） |
...
| `settings.json` | 持久化设置（`{debugLogMaxBytes?: number}`，debug.log 大小上限覆盖） | 全局，跨重启共享 |
| `<serverId>/debug.log` | 后端调试日志（单文件大小上限：settings.json > env > 20MB，超限截断重写） | 按实例（serverId） |
