...
1. **stdio MCP（默认）**：`node build/index.js`，经 stdio 说 MCP 协议，注册 **14 个工具**。
...
- **`/mcp`**：Streamable HTTP 传输的 MCP 端点，**stateless**（每请求新建 transport+server），注册 **13 个工具**（比 stdio 仅少 `process-input`）。
...
- **历史全局、日志按实例隔离**：`processes.json`/`rooms.json`/`settings.json`（历史、房间元数据、持久化设置如 debug.log 大小上限）在数据目录根级，跨重启/跨 server 共享；而 `debug.log` 与每个进程的 `.log` 在 `<serverId>/` 子目录下，按实例隔离。`serverId` 是启动时生成的 6 位 nanoid，每次重启变化。数据目录 = `--data-path` > `PROCM_MCP_DIR` > `<tmpdir>/procm-mcp`。
- **日志双写**：内存环形缓冲（2000 行/流，供 tail/grep）+ 磁盘 append-only `.log`（供停止后仍可读、可下载）。`debug.log` 有单文件大小上限（`settings.json` > env > 20MB），超限整体截断重写；进程 `.log` 暂无上限。
