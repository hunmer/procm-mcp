...
npm run watch              # tsc -w
npm test                   # build + 跑 13 套测试（run-all）
...
- **新增 MCP 工具要在两处都注册**：`index.ts`（stdio）与 `mcp-http.ts` 的 `registerAllTools`（HTTP `/mcp`）。当前差异只剩 `process-input`：stdio 14 个、`/mcp` 13 个（api-operations 组两处都已注册）。
