...
**`/mcp` HTTP（13 个）**：同上仅去掉 `process-input`（api-operations 组三件 8-18 起已在 `/mcp` 注册）。写 stdin/发信号经 HTTP 用 REST `POST /api/processes/:id/input` 或 dashboard。
...
| `process-logs` | `id`(必) `stream?` `pattern?` `count?` `ignoreCase?` | 无 pattern → tail（默认 stdout，count 10）；有 pattern → 正则 grep（默认 50，可搜双流）。结构化日志行（SDK marker）渲染为 `[ts] (stream) LEVEL clientName: message {data}`，非结构化行剥帧后按 `[ts] (stream) message` 输出。 |
...
| `clear-process-logs` | `id`(必) | 清空进程 stdout/stderr 历史（运行中清内存缓冲并截断日志文件，之后继续记录新日志）；清空成功后 `emitLogClear` 广播通知 dashboard；不存在返回 `{error:"Process not found"}`。 |
| `import-process-batch` | `items`(必)[] `group?` | 批量导入进程配置（不启动），每项 `{script,args,cwd,name?,desc?}` 存为 favorite 记录；`group` 应用于每一项。 |
| `select-directory` | `title?` | 弹出系统原生目录选择器（macOS 走 osascript，其他平台 native-file-dialog；选择器在子进程中 `await` 执行，不阻塞事件循环）→ `{canceled,path}`。 |
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/api/processes?group=&status=&roomId=&search=` | 合并内存+历史记录，按 `startedAt` 倒序；可选过滤（AND 组合，空即跳过）：`group`/`roomId` 精确匹配、`status`（spawning/running/exited/error）、`search` 对 name/script/cwd 不区分大小写子串。响应顶层含 `port`（HTTP 实际监听端口，stdio-only 为 null） |
| POST | `/api/processes` | 启动（body `{script,name?,args?[],cwd,envs?,desc?,port?,roomId?,group?}`）→ `{id,name}` |
| PATCH | `/api/processes/:id` | 按需合并可编辑字段：`name`/`script`/`args[]`/`cwd`/`desc`（空串→null）/`port`（1-65535 或 null）/`envs`/`group`（string\|null，空串→null）/`favorite`（boolean，UI 收藏开关）。无可更新字段 400；成功返回公开视图或历史记录 |
...
| DELETE | `/api/processes/:id/logs` | 清空该进程 stdout/stderr 历史（运行中清内存缓冲，历史记录截断日志文件）→ `{id,cleared:true}`；404 `{error:"Process not found"}`；清空后 `emitLogClear` 经 WS 广播 `{type:"logCleared"}` |
| POST | `/api/processes/import` | 单条导入进程配置为 favorite 记录（不启动，dashboard 设置页导入用）→ 201 `{id,name,favorite:true}` |
...
| POST | `/api/select-directory` | body `{title?}` 弹原生目录选择器（子进程异步执行，见 `native-directory.ts`）→ `{canceled,path}`；弹不出 500 |
...
| GET | `/api/server-log` | 服务端调试日志状态：`{dir, maxBytes, defaultMaxBytes, envMaxBytes, files:[{name,path,size,modifiedAt}]}`。`maxBytes` 优先级 `settings.json` > `PROCM_DEBUG_LOG_MAX_BYTES` > 默认 20MB；`files` 为数据目录下各实例的 debug.log（mtime 倒序） |
| PUT | `/api/server-log/settings` | body `{maxBytes?: number\|null}`：正整数字节设置、`null` 清除（回退 env/默认）；持久化到数据根 `settings.json` 立即生效，返回同 GET 结构；非法 400 |
| DELETE | `/api/server-log` | 删除数据目录下所有 server 日志目录（`debug.log` + `processes/` 进程日志）；当前运行实例目录因文件句柄占用改为原地清空 → `{cleared:string[]}` |
...
`POST|GET|DELETE /mcp` → Streamable HTTP，stateless（每请求新建 transport+server），13 工具（仅缺 `process-input`，api-operations 组已注册）。`OPTIONS /mcp` 在 token 检查前做 CORS 预检，反射 Origin。连接配置：`{"type":"http","url":"http://127.0.0.1:<port>/mcp"}`。

## WebSocket（双端点）

- **`/ws`**（dashboard）：连接即发 `{type:"processes",snapshot:true,...}` 快照（顶层含 `port`：HTTP 监听端口或 null）；之后 `PROCESS_CHANGE` → `{type:"processes",...}`（含 `port`），`LOG_APPEND` → `{type:"log",processId,stream,timestamp,message}`，`LOG_CLEAR` → `{type:"logCleared",processId}`。
