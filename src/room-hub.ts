...
import { ensureRoom, listRoomRecords, getRoomRecord, updateRoom, rememberRoomClient, type RoomRecord } from "./room-repository.js";
import { serverLog } from "./server-log.js";
import { putTrace, TraceStoreError } from "./trace-store.js";
import { persistPublishedRoomLog } from "./room-logs.js";
...
export function attachRoomSocket(socket: WebSocket): void {
  ...
  socket.on("message", async (raw) => {
    ...
    if (frame.type === "hello") {
      ...
      await rememberRoomClient(frame.roomId, member);
    ...
    if (frame.type === "publish") {
      ...
      if (frame.topic === PROCM_LOG_TOPIC && !session.member.processId) {
        await persistPublishedRoomLog(session.roomId, session.member, frame.payload as JsonValue);
      }
      const room = sessionsByRoom.get(session.roomId);
      if (room) {
        for (const peer of room.values()) {
...
export async function getRoom(id: string): Promise<RoomView | undefined> {
  ...
  return { ...record, members: mergeMembers(record) };
}

function mergeMembers(record: RoomRecord): RoomMember[] {
  const active = [...(sessionsByRoom.get(record.id)?.values() ?? [])].map((session) => session.member);
  const byId = new Map((record.clientHistory ?? []).map((member) => [member.memberId, member as RoomMember]));
  for (const member of active) byId.set(member.memberId, member);
  return [...byId.values()];
}
