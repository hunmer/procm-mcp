...
export async function listRoomRecords(): Promise<RoomRecord[]> {
  ...
  await db.read();
  return db.data.rooms.map((room) => ({ ...room, processIds: [...room.processIds] }));
}
...
export async function ensureRoom(id: string, processId?: string): Promise<RoomRecord> {
  ...
  const now = Date.now();
  if (!room) {
    room = { id, title: id, note: "", processIds: [], createdAt: now, updatedAt: now };
  ...
  await db.write();
  return { ...room, processIds: [...room.processIds] };
}
