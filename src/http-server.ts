...
import { serverLog, serverId, serverStartedAt, logServerId } from "./server-log.js";
...
// stdio-only mode). Reported via /api/processes and the WS
...
function createRequestHandler(token: string | undefined) {
  return async (req: http.IncomingMessage, res: http.ServerResponse) => {
    try {
      ...
      // GET /api/server-log -> the server's own debug.log size cap (settings >
      // env > default) plus every debug.log file in the data root. Powers the
      // dashboard settings' Logs panel.
      if (method === "GET" && pathname === "/api/server-log") {
        json(res, 200, { dir: ProcmMcpDir(), ...getDebugLogSettings(), files: listDebugLogFiles() });
        return;
      }

      // PUT /api/server-log/settings { maxBytes } -> set (positive integer
      // bytes) or clear (null) the persisted debug.log cap. Clearing falls
      // back to env PROCM_DEBUG_LOG_MAX_BYTES, else the 20MB default.
      if (method === "PUT" && pathname === "/api/server-log/settings") {
        ...
        if (
          ...
          json(res, 400, { error: "maxBytes must be a positive integer or null" });
          return;
        }
        setDebugLogMaxBytes(raw === null ? null : raw);
        json(res, 200, { dir: ProcmMcpDir(), ...getDebugLogSettings(), files: listDebugLogFiles() });
        return;
      }

      // DELETE /api/server-log -> remove every server-log directory in the
      // data root (debug.log + processes/ logs). The current generation's dir
      // is emptied in place (its live process log writers hold open handles).
      // Returns the handled directory names.
      if (method === "DELETE" && pathname === "/api/server-log") {
        json(res, 200, clearDebugLogDirs({ currentServerId: logServerId }));
        return;
      }
